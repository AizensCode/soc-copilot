...
- **Hygiene bundle.** Lazy component-scoped config ✅, CI running the free suite on every push ✅ (see "Tests that run themselves"), and argparse command parsing ✅ — each command now validates its own arguments, so a misspelled `--auto-close` in a systemd unit is **rejected with an error** instead of silently ignored (a swallowed flag silently changes autonomous behavior; the CLI keeps its `--command` shape so every documented invocation still works). Still open: a real package name with a console entry point, and removing the CWD debug-file writes (`last_run_debug.json`, `last_agentic_final_turn.txt`).
...
- **Still a small alert set.** Thirteen labeled samples (two of them ECS-shaped raw Elastic hits) spanning initial access (phishing attachment, credential-phishing link), credential access (SSH brute force), execution/defense evasion (an adversarial encoded-PowerShell injection alert), lateral movement (WMIC remote process creation), persistence (scheduled task from an unsigned AppData binary), cloud identity abuse (CloudTrail IAM privilege escalation), account compromise (impossible-travel login), C2/exfil (DNS tunneling), and two benign false positives (credentialed vulnerability scan, SCCM-scheduled encoded PowerShell). Real SOC environments have dozens of alert classes; this covers most of the kill chain but nothing like the variety of a production queue.
