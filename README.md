...
- **Alert families with benign twins.** Ransomware precursors (shadow-copy deletion) shipped ✅ with a calibrated benign twin (see "Knowing when to say 'this is fine'"). OAuth illicit-consent-grant now shipped ✅ with its benign twin and the `saas_apps` inventory extension it needed (see "The consent grant is the credential") — the asset inventory now grounds sanctioned SaaS legitimacy in the operator's record, exactly as this item anticipated. Still open: WAF-visible web attacks (SQLi/XSS in access logs) with a benign twin.
...
- **Robustness.** Watch-loop hardening (persistent dedupe, retry budgets, backoff, graceful shutdown — and a testable seam: the loop is currently only exercised live, so the closure-ordering fix above rests on review-verified reading rather than a pinned test), one outbound-HTTP policy with bounded retries, and a summary index for the history store before linear JSONL scans hurt. Also named by the scorecard review: the daily digest still can't tell an auto-closed alert from one waiting in the human queue — now that closures are recorded in the store, the digest's queue section could (and should) subtract them.
