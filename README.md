...
- **Eval infrastructure.** ~~Recorded tool cassettes~~ ✅ Implemented — external reputation (AbuseIPDB/VirusTotal/URLScan) is replayed from a committed, faithfully-recorded cassette (`tests/cassette.py`, `data/evals/cassettes/reputation.json`), so a moved verdict is a model change rather than a reputation re-score (see "The harness brings its own reputation, too"). Re-recording proved the drift was never hypothetical — the Tor exit the roadmap called "100/100" now scores 81 — and the increment's own fail-loud guard caught a live gap during review-hardening (a payload host the agentic model discovered through the SIEM double), which is the guard working as designed. Still open in this item: a calibration runner that turns the house 12-run discipline into recorded pass-rate data, and a model-upgrade A/B harness with property-level diffs.
